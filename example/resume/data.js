...
const projects = [
  {
    ...
    qaq: [
      {
        question: '为什么使用antd-pro模版创建项目',
        answer: '这个项目是已经在运营的项目，所以不太清楚当时为什么选用antd-pro（猜测：也许是为了方便）'
      },
      {
        question: '为什么会用到Nginx？',
        answer:
          '因为本地服务和服务端交互的时候会出现跨域问题。由于以前调试方式都是发布到服务器调试这样效率太低为了提高效率所以使用nginx转发了请求解决跨域请求问题，实现本地调试。'
      },
      {
        question: '为什么会有用到shell？',
        answer:
  ...
  },
  {
    ...
    qaq: [
      {
        question: '为什么使用Jquery，没有使用React或Vue？',
        answer: '这个项目是已经在运营的项目，所以不太清楚当时为什么选用Jquery'
      },
      {
        question: '为什么会用到Nginx？',
        answer:
  ...
  },
  {
    ...
    qaq: [
      {
        question: '如何处理树组件（复选框）情况父子级联动问题？',
        answer:
  ...
  },
  {
    ...
    qaq: [
      {
        question: '为什么会使用d3？',
        answer: '为了更好的预览和配置销售团队组织架构。'
      },
      {
        question: '为什么自己手动实现组织架构，而不使用开源？',
        answer: '销售团队组织架构不断的有人员信息更新导致开源库中UI不统一，所以选择自己使用d3自己实现。'
      }
    ]
  },
  {
...

  {
    ...
    description: [
      '规范业务流程和客户信息。',
      '帮助业务人员审核客户订单，替客户下单， 向客户推荐促销、热销产品。',
      '主要模块有： 封闭登录、订单处理、客户管理、产品浏览、（订单|收藏）分享、日程管理、分配客户、调查反馈、流程审批、消息推送。',
      '封闭登录中通过调用系统指纹或者人脸进行二次登录验证。',
      '订单处理中实时反应订单当前状态和物流信息，同时如果未审核订单如果客户有需要更改产品数量业务员可以帮客户修改，也可以将相同的订单分享给另一个需要的客户。',
      '客户管理中实时反应业务员关联的客户状态，拜访信息提醒，客户即将消失提醒。',
      '产品浏览中根据用户不同展示不同的价格、折扣。',
      '（订单|收藏）分享将集团的主打产品或热卖产品分享给需要的客户提高销量。',
      '日程管理每间隔一段时间需要对客户进行回访，回访的同时需要签到，通过地图导航到客户所在的位置。',
      '分配客户将自己所关联的客户分配给下级业务员。',
      '调查反馈通过填写管理端设置的模板信息手机用户诉求和建议。',
      '流程审批下级业务员对客户进行操作（删除、升级、更新信息）都需要上级审批后才能生效。'
    ],
    qaq: [
      {
        question: '产品预览切换分页/分类由于ios&android事件处理机制不一致导致无法实现效果？',
        answer: '通过原生模块封装组件实现列表切换分页/分类'
      },
      {
        question: '应用集成第三方微信库无法分享到小程序？',
        answer: 'fork第三方库增加分享到小程序代码，发布到npm服务器。'
      },
      {
        question: '为什么混合使用RESTful和graphql？',
        answer: '客户端需要同步数据离线使用RESTful无法做到这一点，所以增加graphql请求数据。'
      }
    ]
  }
];
